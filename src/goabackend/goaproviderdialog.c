                               ...
                               GtkWindow *window)
{
  ...
  /* Ensure the close-request handler is called */
...
/**
 * goa_provider_task_return_if_completed:
 * @task: a `GTask`
 *
 * Check if @task is completed or cancelled. If @task is cancelled it will be
 * completed by reporting an error, but only if not already completed.
 *
 * Returns: %TRUE if completed, or %FALSE
 */
gboolean
goa_provider_task_return_if_completed (GTask *task)
{
  g_return_val_if_fail (G_IS_TASK (task), TRUE);

  /* We don't want to override the error if marked as complete */
  if (g_task_get_completed (task))
    return TRUE;
