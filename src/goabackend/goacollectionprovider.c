                         ...
                         GError             **error)
{
  ...
  uri = goa_util_lookup_keyfile_string (object, "FilesUri");
...
typedef struct
{
  ...
  GoaDavConfiguration *dav_config;
...
add_account_data_free (gpointer user_data)
{
  ...
  g_clear_pointer (&data->dav_config, goa_dav_configuration_free);
                         ...
                         const char *name)
{
  for (unsigned int i = 0; i < services->len; i++)
    {
      GoaMailConfig *config = g_ptr_array_index (services, i);

      if (config->server_type != NULL &&
          g_ascii_strcasecmp (config->server_type, name) == 0)
        return config;
    }
                       ...
                       AddAccountData *data)
{
  ...
  page = goa_provider_dialog_add_page (data->dialog,
  ...
  if (row == data->mail_row)
    {
      ...
      if (data->imap_config != NULL)
        {
          gtk_editable_set_text (GTK_EDITABLE (server), data->imap_config->hostname);
          gtk_editable_set_text (GTK_EDITABLE (username), data->imap_config->username);
      ...
      if (data->mail_services != NULL)
        {
          gtk_editable_set_text (GTK_EDITABLE (server), data->smtp_config->hostname);
          gtk_editable_set_text (GTK_EDITABLE (username), data->smtp_config->username);
          gtk_editable_set_text (GTK_EDITABLE (password), "");
        }
    }
  else
    {
      ...
      if (data->dav_config != NULL)
        {
          if (row == data->calendar_row)
            {
              dav_server = data->dav_config->caldav_uri;
              dav_username = gtk_editable_get_text (GTK_EDITABLE (data->email_address));
              dav_password = gtk_editable_get_text (GTK_EDITABLE (data->password));
            }
          else if (row == data->contacts_row)
            {
              dav_server = data->dav_config->carddav_uri;
              dav_username = gtk_editable_get_text (GTK_EDITABLE (data->email_address));
              dav_password = gtk_editable_get_text (GTK_EDITABLE (data->password));
            }
          else if (row == data->files_row)
            {
              dav_server = data->dav_config->webdav_uri;
              dav_username = gtk_editable_get_text (GTK_EDITABLE (data->email_address));
              dav_password = gtk_editable_get_text (GTK_EDITABLE (data->password));
            }
        }
                        ...
                        const char *status)
{
  ...
  else if (g_str_equal (status, "authentication-required"))
    {
      gtk_widget_set_css_classes (box, (const char *[]){ "warning", NULL });
      gtk_label_set_label (GTK_LABEL (label), _("Needs Authentication"));
      gtk_image_set_from_icon_name (GTK_IMAGE (icon), "emblem-important-symbolic");
    }
  else if (g_str_equal (status, "authentication-failure"))
    {
      gtk_widget_set_css_classes (box, (const char *[]){ "warning", NULL });
                 ...
                 gpointer      user_data)
{
  ...
  if (config != NULL)
    {
      g_clear_pointer (&data->dav_config, goa_dav_configuration_free);
      data->dav_config = g_steal_pointer (&config);
    }
  else if (g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
    {
      g_debug ("DISCOVERY CANCELLED");
      goto out;
    }
  else if (g_error_matches (error, GOA_ERROR, GOA_ERROR_NOT_AUTHORIZED))
    {
      ...
      feature_row_set_status (data->calendar_row,
                              has_credentials ? "authentication-failed" : "authentication-required");
      feature_row_set_status (data->contacts_row,
                              has_credentials ? "authentication-failed" : "authentication-required");
      feature_row_set_status (data->files_row,
                              has_credentials ? "authentication-failed" : "authentication-required");
      goto out;
    }
  else
    {
      ...
      feature_row_set_status (data->calendar_row, "error");
      feature_row_set_status (data->contacts_row, "error");
      feature_row_set_status (data->files_row, "error");
      data->pending -= 1;
      goto out;
    }

  feature_row_set_status (data->calendar_row, data->dav_config->caldav_uri != NULL ? "success" : NULL);
  feature_row_set_status (data->contacts_row, data->dav_config->carddav_uri != NULL ? "success" : NULL);
  feature_row_set_status (data->files_row, data->dav_config->webdav_uri != NULL ? "success" : NULL);
  ...
  if (services != NULL)
    {
      ...
      data->imap_config = services_find_preferred (data->mail_services, "imap");
      data->smtp_config = services_find_preferred (data->mail_services, "smtp");
    }
  else if (g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
    {
      goto out;
  ...
  else if (g_error_matches (error, GOA_ERROR, GOA_ERROR_NOT_AUTHORIZED))
    {
      ...
      if ((username == NULL || *username == '\0')
          || (password == NULL || *password == '\0'))
        {
          feature_row_set_status (data->mail_row, "authentication-required");
        }
      else
        {
          feature_row_set_status (data->mail_row, "authentication-failure");
        }
    ...
    {
      g_warning ("%s(): %s", G_STRFUNC, error->message);
      feature_row_set_status (data->mail_row, "error");
      data->pending -= 1;
      goto out;
    }

  feature_row_set_status (data->mail_row, "success");
  data->pending -= 1;
                     ...
                     gpointer     user_data)
{
  ...
  if (password != NULL && *password != '\0')
    {
      if (data->imap_config != NULL || data->dav_config != NULL)
        ready = TRUE;
    }
...
add_account_credentials (GTask *task)
{
  ...
  if (data->dav_config != NULL)
    {
      calendar_enabled = data->dav_config->caldav_uri != NULL;
      contacts_enabled = data->dav_config->carddav_uri != NULL;
      files_enabled = data->dav_config->webdav_uri != NULL;
    }
  ...
  g_variant_builder_add (&details, "{ss}", "Name", g_get_real_name ());
  g_variant_builder_add (&details, "{ss}", "ImapHost", data->imap_config->hostname);
  g_variant_builder_add (&details, "{ss}", "ImapUserName", data->imap_config->username);
  g_variant_builder_add (&details, "{ss}", "ImapUseSsl", (data->imap_config->tls_type == GOA_TLS_TYPE_SSL) ? "true" : "false");
  g_variant_builder_add (&details, "{ss}", "ImapUseTls", (data->imap_config->tls_type == GOA_TLS_TYPE_STARTTLS) ? "true" : "false");
  g_variant_builder_add (&details, "{ss}", "ImapAcceptSslErrors", "false");
  g_variant_builder_add (&details, "{ss}", "SmtpHost", data->smtp_config->hostname);
  g_variant_builder_add (&details, "{ss}", "SmtpUseAuth", (smtp_use_auth) ? "true" : "false");
  if (smtp_use_auth)
    {
      g_variant_builder_add (&details, "{ss}", "SmtpUserName", data->smtp_config->username);
      g_variant_builder_add (&details, "{ss}", "SmtpAuthLogin", "true");
      g_variant_builder_add (&details, "{ss}", "SmtpAuthPlain", "true");
    }
  g_variant_builder_add (&details, "{ss}", "SmtpUseSsl", (data->smtp_config->tls_type == GOA_TLS_TYPE_SSL) ? "true" : "false");
  g_variant_builder_add (&details, "{ss}", "SmtpUseTls", (data->smtp_config->tls_type == GOA_TLS_TYPE_STARTTLS) ? "true" : "false");
  ...
  if (calendar_enabled)
    {
      g_variant_builder_add (&details, "{ss}", "CalDavUri", data->dav_config->caldav_uri);
      g_variant_builder_add (&details, "{ss}", "CalDavUsername", email_address);
      g_variant_builder_add (&details, "{ss}", "CalDavAcceptSslErrors", "false");
    }
  ...
  if (contacts_enabled)
    {
      g_variant_builder_add (&details, "{ss}", "CardDavUri", data->dav_config->carddav_uri);
      g_variant_builder_add (&details, "{ss}", "CardDavUsername", email_address);
      g_variant_builder_add (&details, "{ss}", "CardDavAcceptSslErrors", "false");
    }
  ...
  if (files_enabled)
    {
      g_variant_builder_add (&details, "{ss}", "FilesUri", data->dav_config->webdav_uri);
      g_variant_builder_add (&details, "{ss}", "FilesUsername", email_address);
      g_variant_builder_add (&details, "{ss}", "FilesAcceptSslErrors", "false");
    }
