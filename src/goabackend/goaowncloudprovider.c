...
loginflow2_poll_timeout_cb (gpointer user_data)
{
  ...
  if (!g_cancellable_is_cancelled (data->req_cancellable))
    {
                     ...
                     gpointer      user_data)
{
  ...
  if (response != NULL)
  ...
  else if (g_error_matches (error, GOA_ERROR, GOA_ERROR_SSL))
    {
  ...
  else if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
    {
      ...
      if (g_error_matches (error, G_IO_ERROR, G_IO_ERROR_TIMED_OUT))
        ...
        goa_provider_dialog_add_toast (data->dialog, adw_toast_new (_("Not found")));
    }
}
                  ...
                  AddAccountData *data)
{
  ...
  data->discovery_id = g_timeout_add_once (250, entry_changed_cb, data);
}
                           ...
                           gpointer      user_data)
{
   ...
   */
  webdav_path = g_build_path (G_DIR_SEPARATOR_S, g_uri_get_path (uri), "remote.php/webdav", NULL);
  webdav_uri = g_uri_join (G_URI_FLAGS_PARSE_RELAXED,
                           g_uri_get_scheme (uri),
                           NULL,
                           g_uri_get_host (uri),
                           g_uri_get_port (uri),
  ...
  dav_path = g_build_path (G_DIR_SEPARATOR_S, g_uri_get_path (uri), "remote.php/dav", NULL);
  dav_uri = g_uri_join (G_URI_FLAGS_PARSE_RELAXED,
                        g_uri_get_scheme (uri),
                        NULL,
                        g_uri_get_host (uri),
                        g_uri_get_port (uri),
  ...
  if ((data->flags & GOA_AUTH_FLOW_DO_NOT_LAUNCH_URI) != 0)
    {
      ...
      login_uri = json_object_get_string_member (json_node_get_object (data->auth), "login");
      gdk_clipboard_set_text (gtk_widget_get_clipboard (GTK_WIDGET (dialog)), login_uri);
