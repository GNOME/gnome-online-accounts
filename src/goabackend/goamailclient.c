...
goa_mail_config_new_from_xml (RestXmlNode *server,
                              const char  *email_address,
                              const char  *email_localpart,
                              const char  *email_domain)
{
  GoaMailConfig *config = NULL;
...
/*< private >
 * goa_mail_config_parse_autoconfig_xml:
 ...
 */
static GPtrArray *
goa_mail_config_parse_autoconfig_xml (GBytes      *xml_bytes,
                                      const char  *email_address,
                                      GError     **error)
{
  ...
  services = g_ptr_array_new_with_free_func ((GDestroyNotify)goa_mail_config_free);
  ...
  for (RestXmlNode *node = incoming_server; node != NULL; node = node->next)
    {
      ...
      server = goa_mail_config_new_from_xml (node, email_address, email_localpart, email_domain);
      if (server != NULL)
                               ...
                               gpointer      user_data)
{
  ...
  services = goa_mail_config_parse_autoconfig_xml (body, data->email_address, NULL);
  if (services != NULL)
    {
...
/**
 * goa_mail_config_free:
 * @config: a `GoaMailConfig`
 *
 * Free a mail server configuration.
 */
void
goa_mail_config_free (GoaMailConfig *config)
{
