...
static GoaMailServerConfig *
goa_mail_server_config_new_for_autoconfig_xml (RestXmlNode *server,
                                               const char  *email_address,
                                               const char  *email_localpart,
                                               const char  *email_domain)
{
  GoaMailServerConfig *config = NULL;
  ...
  /* GOA only supports IMAP/SMTP, so all other server types are ignored.
   */
  server_type = rest_xml_node_get_attr (server, "type");
  if (g_strcmp0 (server_type, AUTOCONFIG_SERVER_TYPE_IMAP) != 0
      && g_strcmp0 (server_type, AUTOCONFIG_SERVER_TYPE_SMTP) != 0)
    {
      g_debug ("%s(): ignoring \"%s\" server", G_STRFUNC, server_type);
      return NULL;
    }
  ...
  if (socket_type == NULL || socket_type->content == NULL)
    return NULL;
  else if (g_ascii_strcasecmp (socket_type->content, AUTOCONFIG_SOCKET_TYPE_SSL) == 0)
  ...
  g_return_val_if_fail (hostname != NULL && hostname->content != NULL, NULL);

  config = g_new0 (GoaMailServerConfig, 1);
  config->server_type = g_strdup (server_type);
  config->hostname = g_strdup (hostname->content);
  ...
  if (port != NULL && port->content != NULL)
    {
      ...
      if (port_ > 0 && port_ < UINT16_MAX)
        config->port = (uint16_t)port_;
    }

  if (config->port == 0)
    {
      if (g_ascii_strcasecmp (server_type, AUTOCONFIG_SERVER_TYPE_IMAP) == 0)
        config->port = (tls_type == GOA_TLS_TYPE_SSL) ? 993 : 143;
      else if (g_ascii_strcasecmp (server_type, AUTOCONFIG_SERVER_TYPE_SMTP) == 0)
        config->port = (tls_type == GOA_TLS_TYPE_SSL) ? 465 : 587;
    }
...
/*< private >
 * goa_mail_config_new_for_autoconfig_xml:
 ...
 *
 * Returns: (transfer container) (element-type Goa.MailServerConfig): a list of discovered services
 */
static GPtrArray *
goa_mail_config_new_for_autoconfig_xml (GBytes      *xml_bytes,
                                        const char  *email_address,
                                        GError     **error)
{
  ...
  services = g_ptr_array_new_with_free_func ((GDestroyNotify)goa_mail_server_config_free);
  ...
  if (email_node == NULL)
    {
      g_set_error_literal (error,
                           ...
                           "Missing \"emailProvider\" in Autoconfig XML");
      return NULL;
    }
  ...
  for (RestXmlNode *node = incoming_server; node != NULL; node = node->next)
    {
      GoaMailServerConfig *server = NULL;

      server = goa_mail_server_config_new_for_autoconfig_xml (node,
                                                              email_address,
                                                              email_localpart,
                                                              email_domain);
      if (server != NULL)
                               ...
                               gpointer      user_data)
{
  ...
  services = goa_mail_config_new_for_autoconfig_xml (body, data->email_address, NULL);
  if (services != NULL)
    {
...
/**
 * goa_mail_server_config_free:
 * @config: a `GoaMailServerConfig`
 *
 * Free a mail server configuration.
 */
void
goa_mail_server_config_free (GoaMailServerConfig *config)
{
  g_return_if_fail (config != NULL);

  g_clear_pointer (&config->server_type, g_free);
  g_clear_pointer (&config->hostname, g_free);
