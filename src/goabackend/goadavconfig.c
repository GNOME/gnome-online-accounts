...
static void
goa_dav_config_build_variant (GoaServiceConfig *config,
                              GVariantBuilder  *builder)
{
  GoaDavConfig *self = GOA_DAV_CONFIG (config);
  const char *service = "unknown";

  g_return_if_fail (GOA_IS_DAV_CONFIG (self));
  g_return_if_fail (builder != NULL);

  service = goa_service_config_get_service (config);
  if (g_str_equal (service, GOA_SERVICE_TYPE_CALDAV))
    {
      g_variant_builder_add (builder, "{ss}", "CalDavUri", self->uri);
      g_variant_builder_add (builder, "{ss}", "CalDavUsername", self->username);
    }
  else if (g_str_equal (service, GOA_SERVICE_TYPE_CARDDAV))
    {
      g_variant_builder_add (builder, "{ss}", "CardDavUri", self->uri);
      g_variant_builder_add (builder, "{ss}", "CardDavUsername", self->username);
    }

  else if (g_str_equal (service, GOA_SERVICE_TYPE_WEBDAV))
    {
      g_variant_builder_add (builder, "{ss}", "WebDavUri", self->uri);
      g_variant_builder_add (builder, "{ss}", "WebDavUsername", self->username);
    }
}
...
goa_dav_config_class_init (GoaDavConfigClass *klass)
{
  ...
  config_class->build_variant = goa_dav_config_build_variant;
