...
on_principal_changed (GtkEditable *editable, gpointer user_data)
{
  ...
  principal = gtk_entry_get_text (GTK_ENTRY (data->principal));
                           ...
                           AddAccountData    *data)
{
  ...
  row = 0;
  add_entry (grid1, row++, _("_Principal"), &data->principal);
                                               ...
                                               GTask              *operation_result)
{
  ...
  self = GOA_FEDORA_PROVIDER (g_task_get_source_object (operation_result));
...
static void
perform_initial_sign_in (GoaFedoraProvider   *self,
                         GoaObject           *object,
                         const char          *principal,
                         GCancellable        *cancellable,
                         GAsyncReadyCallback  callback,
...
static gboolean
perform_initial_sign_in_finish (GoaFedoraProvider  *self,
                                GAsyncResult       *result,
                                GError            **error)
{
  GTask *task;

  g_return_val_if_fail (GOA_IS_FEDORA_PROVIDER (self), FALSE);
...
static void
on_account_signed_in (GoaFedoraProvider  *self,
                      GAsyncResult       *result,
                      AddAccountData     *data)
{
  perform_initial_sign_in_finish (self, result, &data->error);
...
static void
goa_fedora_provider_class_init (GoaFedoraProviderClass *kerberos_class)
{
