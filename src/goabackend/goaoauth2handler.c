...
get_oauth2_provider (const char  *needle)
{
  ...
  for (unsigned int i = 0; oauth2_providers[i] != NULL; i++)
    {
      if (g_str_equal (needle, oauth2_providers[i]))
        return oauth2_providers[i];
    }
  ...
  if (scheme != NULL)
    {
      ...
      client_id = get_oauth2_provider (tmp->str);
    }
      ...
      if (path != NULL && *path != '\0')
        {
          ...
          client_id = get_oauth2_provider (strv[0]);
        }
    }
